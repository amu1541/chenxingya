...
var transporter = nodemailer.createTransport({
    ...
    auth: {
        user: '2281131072@qq.com',
        //这里密码不是qq密码，是你设置的smtp密码
        pass: 'xubtuivmnhlvdjab'
        // pass:'xsiawpkxcoudecac'
    }
});
...
router.post("/message", function(req, res, next) {
    ...
    sqlStateFn(req, res, next, sql).then(function(data) {
        ...
        // 发送邮件
        var mailOptions = {
            from: '2281131072@qq.com', // 发件地址
            to: '2281131072@qq.com', // 收件列表
...
router.get("/meslist", function(req, res, next) {
        var sql = `select * from client_msg`
        console.log(sql)
        query(sql, function(err, rows, fields) {
            if (err) {
                res.send({
